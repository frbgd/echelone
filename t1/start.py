...
interface_name_ = ''
for i_ in net_:
    if str(i_['color']) == str(data_['COLOR']) and str(i_['number']) == str(data_['NUMBER']) and str(
            i_['address']) == str(data_['ADDRESS']):
...
table_data_ = [['Имя', 'Описание', 'Тип', 'Параметр', 'Ожидаемый', 'Код post', 'Фактический (sysifaces)',
                'Фактический (netstatus)', 'Результат']]
...
for i_ in dictionary_:
    ...
    table_data_.append(
        [i_['name'], i_['description'], i_['type'], data_['ADDRESS'], i_['valid'], code_post_, address_sysifaces_,
         address_netstatus_, diff_status_])
    # Прогресс бар
